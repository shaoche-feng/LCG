...
from lcg.crn import make_crn_bank  # noqa: E402
...
def test_torch_cpu_global_rng_preserved_by_candidate_bank(tiny_denoiser):
    ...
    torch.manual_seed(456)
    make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=torch.device("cpu"), num_samples=4, seed=999)
...
def test_torch_cuda_global_rng_preserved_by_candidate_bank(tiny_denoiser):
    ...
    torch.cuda.manual_seed(456)
    make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=device, num_samples=4, seed=999)
...
def test_same_seed_bank_is_identical(tiny_denoiser):
    ...
    bank1 = make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=torch.device("cpu"), num_samples=3, seed=7)
    bank2 = make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=torch.device("cpu"), num_samples=3, seed=7)
...
def test_different_seed_bank_is_different(tiny_denoiser):
    ...
    bank1 = make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=torch.device("cpu"), num_samples=3, seed=7)
    bank2 = make_forward_jvp_simple_mc_bank(TINY_SIGMA_CFG, y_shape, d_S, device=torch.device("cpu"), num_samples=3, seed=8)

    assert not all(torch.equal(a, b) for a, b in zip(bank1.etas, bank2.etas))


def test_legacy_crn_bank_same_seed_reproducible_and_isolated():
    y_shape = torch.Size([1, IMG_CHANNELS, IMG_SIZE, IMG_SIZE])
    torch.manual_seed(111)
    expected_global = torch.rand(10)

    torch.manual_seed(111)
    bank1 = make_crn_bank(TINY_SIGMA_CFG, y_shape, device=torch.device("cpu"), num_strata=3, seed=55)
    actual_global = torch.rand(10)
    bank2 = make_crn_bank(TINY_SIGMA_CFG, y_shape, device=torch.device("cpu"), num_strata=3, seed=55)

    assert torch.equal(actual_global, expected_global)  # global RNG untouched
    for a, b in zip(bank1.epsilons, bank2.epsilons):
        assert torch.equal(a, b)  # same seed -> same bank
...
def test_historical_precision_reproducible(tiny_denoiser, small_dataset):
    ...
    h_D_1 = historical_precision(denoiser, params, dataset, TINY_SIGMA_CFG, B=N, N=N, num_strata=3, seed=99)
    h_D_2 = historical_precision(denoiser, params, dataset, TINY_SIGMA_CFG, B=N, N=N, num_strata=3, seed=99)
...
def test_diamond_torch_sequence_independent_of_historical_precision(tiny_denoiser, small_dataset):
    ...
    torch.manual_seed(S)
    h_D = historical_precision(denoiser, params, dataset, TINY_SIGMA_CFG, B=N, N=N, num_strata=3, seed=17)
...
def test_diamond_numpy_sequence_independent_of_historical_precision(tiny_denoiser, small_dataset):
    ...
    np.random.seed(S)
    historical_precision(denoiser, params, dataset, TINY_SIGMA_CFG, B=N, N=N, num_strata=3, seed=17)
...
def test_integration_smoke_rng_preserved_and_outputs_valid(tiny_denoiser, small_dataset):
    ...
    h_D = historical_precision(denoiser, params, dataset, TINY_SIGMA_CFG, B=N, N=N, num_strata=3, seed=5)
