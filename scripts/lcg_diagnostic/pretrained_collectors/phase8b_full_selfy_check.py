"""Phase 8b -- broader check following the Phase 8a pilot: does the
walk_scarce-specific dampening under self-generated y (seen for
walker/Seed A) generalize across all 3 conditions and both domains, or was
it specific to that one domain/seed?

Same methodology as phase8a_selfgenerated_y_pilot.py (production
DiffusionSampler y*, real conditioning context, identical h_D/theta_S/bank/
held-out candidates to Phase 5), but broadened to all 2 domains x 3
conditions (Seed A only) and using a SINGLE self-y sampling seed per
combination (not 3) since the pilot already established seed-to-seed
stability of the self-y result -- this trades some per-seed variance
information for 3x the domain/condition coverage in similar wall-clock time.

Run from the LCG/ project root:
    python scripts/lcg_diagnostic/pretrained_collectors/phase8b_full_selfy_check.py
"""
...
Y_SAMPLE_SEED = 24681  # reuse the pilot's first seed for continuity
NUM_BOOTSTRAP = 10_000
BOOTSTRAP_SEED = 999

OUT_ROOT = _PROJECT_ROOT / "docs" / "lcg_undersample_diagnostic" / "phase8_imagined_rollout" / "phase8b_full_selfy_check"
...
def process_domain_condition(domain: str, condition: str, candidates: list, sigmas, order: float, device) -> dict:
    print(f"\n{'=' * 70}\n{domain}/{condition}\n{'=' * 70}", flush=True)
    ...
    with open(out_dir / "per_transition_scores.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["domain", "behavior", "episode_id", "transition_index",
        ...
        for beh, eid, tidx, r_score, s_score in zip(behaviors, episode_ids, transition_indices, real_scores, selfy_scores):
            writer.writerow({"domain": domain, "behavior": beh, "episode_id": int(eid), "transition_index": int(tidx),
                              "real_y_lcg_score": float(r_score), "self_y_lcg_score": float(s_score)})
    print(f"  Saved: {out_dir / 'per_transition_scores.csv'}", flush=True)
...
def main() -> None:
    ...
    for domain in DOMAINS:
        ...
        for condition in CONDITIONS:
            ...
            all_results[key] = process_domain_condition(domain, condition, candidates, sigmas, diffusion_cfg.order, device)
            (OUT_ROOT / "phase8b_summary.json").write_text(json.dumps(all_results, indent=2, default=str))

        del candidates
        torch.cuda.empty_cache()

    print("\n" + "=" * 90)
    print("PHASE 8b SUMMARY: real-y Delta vs self-y Delta (single seed), all domain/conditions")
    print("=" * 90)
    for key, r in all_results.items():
        print(f"{key:<25} real_y={r['real_y']['delta_mean']:+9.4f} (sig={r['real_y']['significant']})   "
    ...
    print(f"\nSaved: {OUT_ROOT / 'phase8b_summary.json'}")
